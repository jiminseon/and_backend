...
public class RabbitMQConfig {
    ...
    public static final String FCM_TOKEN_QUEUE = "alert.fcm-token.queue";
    ...
    public static final String FCM_TOKEN_ROUTING_KEY = "user.fcm-token.changed";
    ...
    @Bean
    public Binding alertTestBinding(
            @Qualifier("alertTestQueue") Queue alertTestQueue,
            @Qualifier("alertExchange") DirectExchange alertExchange) {
    ...
    @Bean
    public Queue fcmTokenQueue() {
        return QueueBuilder.durable(FCM_TOKEN_QUEUE).quorum().build();
    }


    @Bean
    public Binding companyBinding(
            @Qualifier("alertCompanyQueue") Queue alertCompanyQueue,
            @Qualifier("alertExchange") DirectExchange alertExchange) {
    ...
    public Binding conditionBinding(
            @Qualifier("alertConditionQueue") Queue alertConditionQueue,
            @Qualifier("alertExchange") DirectExchange alertExchange) {
        return BindingBuilder.bind(alertConditionQueue).to(alertExchange).with(ALERT_CONDITION_ROUTING_KEY);
    }

    @Bean
    public Binding fcmTokenBinding(
            @Qualifier("fcmTokenQueue") Queue fcmTokenQueue,
            @Qualifier("alertExchange") DirectExchange alertExchange) {
        return BindingBuilder.bind(fcmTokenQueue).to(alertExchange).with(FCM_TOKEN_ROUTING_KEY);
    }
