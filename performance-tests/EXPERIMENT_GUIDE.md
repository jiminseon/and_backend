...
추가한 코드의 역할과 내부 흐름은 `performance-tests/CODE_WALKTHROUGH.md`에 따로 정리했습니다.
...
| 파일 | 역할 |
| --- | --- |
| `performance-tests/docker-compose.yml` | Redis, RabbitMQ, MySQL, Prometheus, InfluxDB, Grafana 실행 |
| `performance-tests/mysql/01-schema.sql` | 엔티티 기준 MySQL 테이블 생성 |
| `performance-tests/mysql/02-seed.sql` | 사용자, FCM 토큰, 회사, 알림, 캔들 가상 데이터 생성 |
| `performance-tests/scripts/seed-redis.sh` | 알림 평가가 읽는 Redis metric 생성 |
| `performance-tests/k6/alert-detect.js` | 알림 조건 평가 부하테스트 |
| `performance-tests/k6/notification-multicast.js` | FCM multicast 부하테스트 |
| `performance-tests/k6/notification-send-mode.js` | FCM multicast vs 단건 반복 전송 비교 |
| `services/alert-service/src/main/resources/application-loadtest.yml` | 로컬 부하테스트용 Spring profile |
...
```bash
docker compose -f performance-tests/docker-compose.yml up -d
```

실행 후 확인합니다.

```bash
docker compose -f performance-tests/docker-compose.yml ps
```
...
```bash
docker exec -it $(docker compose -f performance-tests/docker-compose.yml ps -q mysql) mysql -uadmin -pandand123123! andDB
```
...
```bash
docker compose -f performance-tests/docker-compose.yml down -v
docker compose -f performance-tests/docker-compose.yml up -d
```
...
```bash
bash performance-tests/scripts/seed-redis.sh
```
...
```bash
K6_OUT=influxdb=http://localhost:8086/k6 k6 run performance-tests/k6/alert-detect.js
```
...
```bash
SEND_MODE=multicast TOKEN_COUNT=500 K6_OUT=influxdb=http://localhost:8086/k6 k6 run performance-tests/k6/notification-send-mode.js
SEND_MODE=single-loop TOKEN_COUNT=500 K6_OUT=influxdb=http://localhost:8086/k6 k6 run performance-tests/k6/notification-send-mode.js
```

토큰 수를 바꿔 추가 실험을 하고 싶으면:

```bash
SEND_MODE=multicast TOKEN_COUNT=100 K6_OUT=influxdb=http://localhost:8086/k6 k6 run performance-tests/k6/notification-send-mode.js
SEND_MODE=single-loop TOKEN_COUNT=100 K6_OUT=influxdb=http://localhost:8086/k6 k6 run performance-tests/k6/notification-send-mode.js
```
...
```bash
docker exec -it $(docker compose -f performance-tests/docker-compose.yml ps -q redis) redis-cli keys '*:*'
```
...
```bash
env JAVA_HOME=$(/usr/libexec/java_home -v 17) ./gradlew :alert-module:compileJava
```
